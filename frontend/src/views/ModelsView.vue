...
import HeatStrip from '../components/common/HeatStrip.vue'
import {
...
// v1.7.648 亮点: 逐月战绩热带 —— 每月胜率≥50%记胜(红), <50%记负(绿); 守 A 股红盈绿亏
function monthlyHeat(monthly?: ModelMonthlyPoint[]): (1 | -1)[] {
  return (monthly || []).map(m => (m.win_rate >= 50 ? 1 : -1))
}
...
<div class="mp-main">
  ...
  <div class="mp-card" v-if="winrateRanked.length"><div class="mp-card-t">🏆 近3月胜率榜<span class="mp-rk-sub">全市场 · 5分钟真实可成交口径 · 真实出场/扣费 · 每晚21:00更新<span v-if="winrate.run_date"> · 截至 {{ winrate.run_date }}</span></span></div><div class="mp-tw"><table class="mp-tbl mp-rk"><thead><tr class="mp-rk-grp"><th rowspan="2">排名</th><th rowspan="2">模型</th><th colspan="3">近3月战绩</th><th colspan="3" class="bg6">近6月战绩</th><th rowspan="2">对比收益<br><span class="mp-th-sub">3月 − 6月</span></th><th colspan="2">周度回测参考</th><th colspan="2" class="bg6">风险画像</th></tr><tr><th>胜率</th><th>单笔<br>均收益</th><th class="sep3">样本</th><th class="bg6">胜率</th><th class="bg6">单笔<br>均收益</th><th class="bg6">样本</th><th :title="METRIC_TIPS.annualized">年化效率 ⓘ</th><th :title="METRIC_TIPS.pf">盈利因子 ⓘ</th><th class="bg6">逐月胜率<br><span class="mp-th-sub">近6月</span></th><th class="bg6" :title="METRIC_TIPS.max_drawdown">最大回撤 ⓘ</th></tr></thead><tbody><tr v-for="r in winrateRanked" :key="r.signal_id"><td><span class="mp-rank-badge" :class="{ gold: r.rank_3m === 1 }">{{ r.rank_3m }}</span></td><td class="cell-name" @click="scrollTo(r.signal_id)">{{ r.model_name }}</td><td class="wr-cell"><span class="wr-num">{{ r.win_rate_3m?.toFixed(1) }}%</span><span v-if="r.win_rate_3m != null" class="wr-mini"><i :class="{ weak: r.win_rate_3m < 40 }" :style="{ width: Math.min(100, r.win_rate_3m) + '%' }" /></span></td><td :class="(r.net_3m || 0) >= 0 ? 'cell-up' : 'cell-down'">{{ fmtNet(r.net_3m) }}</td><td class="sep3">{{ r.n_3m }}笔</td><td class="bg6">{{ r.win_rate_6m != null ? r.win_rate_6m.toFixed(1) + '%' : '—' }}</td><td class="bg6" :class="(r.net_6m || 0) >= 0 ? 'cell-up' : 'cell-down'">{{ fmtNet(r.net_6m) }}</td><td class="bg6">{{ r.n_6m || 0 }}笔</td><td :class="(r.net_3m||0) >= (r.net_6m||0) ? 'cell-up' : 'cell-down'">{{ fmtDelta3v6(r) }}</td><td class="cell-up"><template v-if="btOf(r.signal_id)">+{{ btOf(r.signal_id)!.annualized }}%</template><span v-else>—</span></td><td><template v-if="btOf(r.signal_id)">{{ btOf(r.signal_id)!.pf }}</template><span v-else>—</span></td><td class="bg6"><span v-if="r.monthly && r.monthly.length >= 2" :title="sparkTitle(r.monthly)"><HeatStrip :results="monthlyHeat(r.monthly)" :cell="11" :show-legend="false" /></span><span v-else class="mp-dim">—</span></td><td class="bg6"><span :class="r.max_drawdown != null ? 'cell-down' : 'mp-dim'">{{ ddLabel(r.max_drawdown) }}</span></td></tr></tbody></table></div><p class="mp-sec-foot">全市场回测模型参与排名 · 近3月不含最近约2周触发 · 盘中触发的突破型(缩量突破/回踩MA/平台/强势起点/竞价)按5分钟真实可成交口径(不再用全天收盘筛交易),弱势极限为收盘价入场故按收盘口径 · 年化效率/盈利因子来自周度日线回测仅供趋势参考 · 最大回撤按「同日信号资金等分、逐批复利」的组合口径(不随触发次数膨胀,可跨模型横比),口径于 v1.7.716 修正后当晚 21:00 起重算</p></div>
...
/* 数字/短标签一律不换行: 原先"110笔"、逐月热带都会被挤成两行, 行高参差 */
.mp-tbl td{padding:6px 8px;text-align:center;border-bottom:1px solid #f0f2f5;color:var(--text1);vertical-align:middle;white-space:nowrap}
/* v1.7.716 胜率榜 13 列过密 → 二级表头分组(近3月/近6月/回测参考/风险画像), 让眼睛先认组再读列 */
.mp-rk-grp th{background:#eef2f7;color:#475569;font-size:10.5px;letter-spacing:.3px;border-bottom:1px solid var(--border);padding:5px 8px}
.mp-rk-grp th[rowspan]{background:#f6f8fa;vertical-align:bottom;padding-bottom:7px}
.mp-rk-grp th[colspan]{border-left:1px solid #dde3ea;border-right:1px solid #dde3ea}
.mp-th-sub{display:block;font-weight:400;font-size:9.5px;color:#94a3b8;margin-top:1px}
...
/* v1.7.648 亮点: 近3月胜率单元格 —— 数字 + 胜率迷你条 */
.wr-cell{min-width:52px}
.wr-num{display:block;font-weight:600;line-height:1.2}
.wr-mini{display:block;height:4px;margin-top:3px;border-radius:3px;background:var(--bg-default,#f0f2f5);overflow:hidden}
.wr-mini i{display:block;height:100%;border-radius:3px;background:linear-gradient(90deg,var(--accent-fg,#0969da),#4FA8FF)}
.wr-mini i.weak{background:linear-gradient(90deg,var(--warn-fg,#9A6700),#C9A227)}
...
/* 表格: 保留 .mp-tw 横滚容器, 首列 sticky 锁定。
   v1.7.716 胜率榜改二级表头后必须限定 thead 第一行 —— 否则第二行的"胜率"也是 :first-child,
   会被一起钉死在左边缘, 手机横滚时和排名列叠在一起。 */
.mp-tbl thead tr:first-child th:first-child,.mp-tbl td:first-child{position:sticky;left:0;z-index:1;background:#fff}
.mp-tbl thead tr:first-child th:first-child{background:#f6f8fa}
