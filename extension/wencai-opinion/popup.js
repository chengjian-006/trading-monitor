...
function save() {
  const data = {
    ...
    deepResearch: $('deepResearch').checked,
    autoUpload: $('autoUpload').checked,
    onlyWithStock: $('onlyWithStock').checked,
    schedule: {
...
function runBg(question) {
  ...
  chrome.runtime.sendMessage({ type: 'runBg', question }, (resp) => {
    ...
    if (resp && resp.ok) {
      ...
      else { st.className = 'status ok'; st.textContent = '✓ 已存档：' + ((r.stocks || []).join('、') || '未识别出个股') + '（' + (r.answerLen || 0) + '字）'; }
