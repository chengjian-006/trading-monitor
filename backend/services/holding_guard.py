...
def build_near_high_msg(name: str, code: str, price: float, ph: float, ph_date: str):
    """接近前高卡(基线 v1.1) → card_kit.Card, family=exit(离场族·🛡️减仓语义的持仓守护提示)。
    结论行(距前高 X%) + 强度条 + 建议; 价格长值(现价/前高)下沉折叠。"""
    ...
    elements = [
        card_kit.heading_md(f"距{WINDOW_HIGH}日前高 {card_kit.pct_md(dist * 100)}"),
        md_element(f"现价 {card_kit.strength_bar(fill)} 前高"),
        card_kit.advice("放量站上是突破，缩量到这是压力"),
        card_kit.fold("价格明细", f"现价 ¥{price:.2f}\n"
                                 f"{WINDOW_HIGH}日前高 ¥{ph:.2f}（{ph_date[5:]}）\n"
                                 f"距离 **{dist * 100:+.1f}%**（触发线 ≤{NEAR_HIGH_TOL * 100:.0f}%）"),
    ]
    title = f"🛡️ 接近前高 · {name}({code})"
    fallback = (f"🛡️ {name}({code}) 接近前高\n"
    ...
    return card_kit.Card(
        ...
        summary=card_kit.summary_text(name, code, "接近前高", f"距{dist * 100:+.1f}%"),
        subtitle=f"近{WINDOW_HIGH}日波段阻力 · 持仓守护提示",
        tags=[("持仓守护", "green")],
    )


def build_profit_protect_msg(name: str, code: str, peak_gain: float, cur_gain: float,
                             cost: float, advisory: str = "", model_name: str = ""):
    """盈利保护卡(基线 v1.1) → card_kit.Card, family=exit(离场族·🛡️减仓语义)。
    KPI三栏(当前盈利/峰值盈利/已回吐) + 强度条 + 建议; 价格长值(成本)下沉折叠; 保留「建仓信息」折叠。"""
    from backend.services import card_kit
    from backend.services.lark_notifier import md_element
    ...
    elements = [
        card_kit.kpi_row([
            ("当前盈利", f"{cur_gain * 100:+.1f}%", "red" if cur_gain > 0 else "green"),
            ("峰值盈利", f"{peak_gain * 100:+.1f}%", "red"),
            ("已回吐", f"{giveback:.0f}%", "green"),
        ]),
        md_element(f"成本 {card_kit.strength_bar(fill)} 峰值"),
        card_kit.advice("别让赚过的变成亏的，考虑锁利"),
        card_kit.fold("价格明细", f"成本 ¥{cost:.2f}\n"
                                 f"峰值浮盈 **{peak_gain * 100:+.1f}%** → 当前 **{cur_gain * 100:+.1f}%**\n"
                                 f"已回吐利润 **{giveback:.0f}%**"),
    ]
    if model_name or advisory:
        detail = f"建仓模型: {model_name}" if model_name else ""
        detail = (detail + ("\n" + advisory if advisory else "")).strip()
        elements.append(card_kit.fold("建仓信息", detail))
    title = f"🛡️ 盈利保护 · {name}({code})"
    ...
    return card_kit.Card(
        title=title, elements=elements, fallback=fallback, family="exit",
        summary=card_kit.summary_text(name, code, "盈利保护",
                                      f"峰值{peak_gain * 100:+.1f}%→现{cur_gain * 100:+.1f}%"),
        subtitle="曾大幅获利，已回吐到成本线附近",
        tags=[("持仓守护", "green"), (f"回吐{giveback:.0f}%", "green")],
    )
...
async def _send_card(card):
    try:
        await notifier.send_card(card)
    except Exception as e:
        logger.warning(f"[holding_guard] 推送失败({card.title}): {e}")
...
async def _check_anomaly(code: str, name: str, q: dict, today: str, now_ts: float):
    ...
    if delta is not None:
        ...
        if (delta >= SURGE_PP and not _throttle.throttled(code, "surge", today, limit=SURGE_MAX_DAILY)
            ...
            hits.append(ha.build_surge_card(name, code, delta, win_min, price, pct))
    ...
    if side and sealed is not False:   # 封死, 或无五档(未知)仍报基础涨跌停
        ...
        if not _throttle.throttled(code, rule, today):
            if side == "up":
                hits.append(ha.build_limit_up_card(name, code, price, pct, amt, vol_ratio, amount))
        ...
        if has_l1:
            ...
            if weaken_hit or vol_hit:
                hits.append(ha.build_board_anomaly_card(
                    name, code, side,
...
async def holding_guard_tick():
    ...
    for code in codes:
        ...
        if ph and is_near_high(price, ph) and not _throttle.throttled(code, "prior_high", today):
            try:
                card = build_near_high_msg(name, code, price, ph, ph_date)
                sold_md = pp.build_mark_sold_md(site, 1, code, name)
                if sold_md:                      # 快捷动作行(已卖出)永远最后
                    from backend.services.lark_notifier import md_element as _md_el
                    card.elements = list(card.elements) + [_md_el(sold_md)]
                    card.fallback += "\n" + sold_md
                await notifier.send_card(card)
