...
(WE_NEAR / R10_NEAR / R20_NEAR)是本模块独有的放宽窗口(容差带 ×~1.5)。
"""
import logging
...
# 接近档"贴线带" — 比触发容差(±2%/±1%/±3%)放宽约 1.5 倍, 离均线 7%~13% 的不算接近
WE_NEAR_PCT = 3.0    # 弱势极限锚点(MA10∪MA20)±2% → 近≤3%
R10_NEAR_PCT = 2.0   # 回踩10MA缩量后突破昨高 ±1% → 近≤2%
R20_NEAR_PCT = 4.5   # 回踩20MA缩量后突破昨高 ±3% → 近≤4.5%
PB_NEAR_PCT = 3.0    # 中继平台突破: 现价距突破价(上沿×1.005)还差≤3% = 逼近上沿(尾盘才收盘确认, 全天提前报)
