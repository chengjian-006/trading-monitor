...
请基于以上策略体系和以下市场数据生成盘面分析报告，用纯 HTML 格式输出（禁止用 markdown）。报告只输出【标题栏 + 操作建议判定 + 一句理由】，不要输出热点板块、风险警示、持仓动态、操作要点等任何其他板块。
...
本报告只产出一个核心判定: 适合买入 / 谨慎观望 / 不适合买入 + 一句理由。**不要**输出热点板块、风险警示、持仓动态、操作要点、板块表格、个股清单等任何其他内容。
...
再次强调: 报告到上面这句理由为止, 后面不要再输出任何板块、表格、持仓、操作要点、风险警示等内容。
