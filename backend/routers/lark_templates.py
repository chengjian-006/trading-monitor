...
仍为手工镜像的卡(各处注释标明"手工镜像"):
  - market_risk_controller 状态卡 3 张: 待改直调 _push_state_card 版式
    (退潮提示卡已随 emit_risk_dimension 删除, v1.7.752)
...
register("买入信号", "回踩10MA缩量后突破昨高",
         ...
         _signal_card(
             ...
             detail=("🔴 大盘危险档 · 大盘整体走弱，但该模型在此档历史上接近打平"
                     "(PF≈1.0)，若做务必轻仓\n"
...
# 手工镜像 · market_risk_controller 状态卡 3 张, 待改直调 _push_state_card 版式对齐。
# v1.7.752 (Deploy 2B): 档名「空仓」→「危险」; 盘中监测口径自选池→全市场(涨跌家数+三大指数)。
register("风险预警", "市场风险·危险(升级)",
         "跌破危险线, 状态升到最高危档(手工镜像·待改直调)",
         _v2("🔴 市场风险 · 升到「危险」档", [
             _md("**🟡 谨慎　→　🔴 危险（最高危）**"),
             _md("盘面大跌：全市场5083只，只 **18%** 在涨、三大指数平均 **-1.82%**"),
             _md("<font color='grey'>触发线：<22%在涨 且 三大指数平均跌超1.5% = 危险</font>"),
             card_kit.advice("立即停开新仓、别抄底，今天先保命"),
         ], "red", summary="市场风险 谨慎→危险 只18%在涨", tags=[("危险", "red")]),
         "10:00-14:30每5分(缓冲带+冷静期防打脸)+14:40预升级+16:40复核")

register("风险预警", "市场风险·谨慎(升级)",
         "盘面转弱, 升到谨慎档(手工镜像·待改直调)",
         _v2("🟡 市场风险 · 升到「谨慎」档", [
             _md("**🟢 正常　→　🟡 谨慎（谨慎档）**"),
             _md("盘面转弱：全市场5083只，只 **26%** 在涨、三大指数平均 **-1.20%**"),
             _md("<font color='grey'>触发线：<28%在涨 或 三大指数平均跌超1.0% = 谨慎</font>"),
...
register("风险预警", "市场风险·降到谨慎(降级)",
         "明显转好过退出缓冲线才降档, 措辞诚实不叫回暖(手工镜像·待改直调)",
         _v2("🟡 市场风险 · 降到「谨慎」档", [
             _md("**🔴 危险　→　🟡 谨慎（谨慎档）**"),
             _md("跌势明显缓和：全市场5083只，**42%** 在涨、三大指数平均 **-0.60%**\n——是没那么急了，不是转多。"),
             card_kit.advice("危险警报解除，可小仓试错、别重仓"),
         ], "yellow", summary="市场风险 危险→谨慎 跌势缓和", tags=[("谨慎", "orange")]),
         "10:00-14:30每5分 · 涨跌比≥35%且指数均≥-1.0% 且距上次变档≥30分钟才降")
...
register("风险预警", "预警解除(标准解除卡)",
         ...
         _card_json(card_kit.dismiss_card(
             "市场风险·危险预警", issued_str="07-10 09:35", days_active=4,
             condition_md="全市场涨跌比 **46%** ≥ 45% 且三大指数平均 **-0.2%** ≥ -0.3%（过缓冲带，防贴线反复）",
             period_md="生效期间 上证 **-2.1%**，全市场平均 **-3.4%**，期间买点信号 0 开仓",
             advice_text="警报解除，可逐步恢复试仓",
         )),
         "状态型预警(危险/情绪冰点)结束时收尾 · 防抖=超缓冲带或持续N周期 · 一次性事件卡不发解除")
