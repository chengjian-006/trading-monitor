...
register("盘面分析", "竞价分析·板块强弱", "9:26集合竞价撮合后推板块强弱硬数据卡",
         _card_v2("📊 竞价分析 · 06-15", "blue", [
             ...
             # 竞价最强 TOP5: 2列(涨幅|板块·领涨), 涨幅前置独占短列, 领涨命中自选标红
             _md("🔥 **竞价最强 TOP5**（28行业 + 86概念）"),
             _md(_mdtable(
                 [{"name": "pct", "display_name": "涨幅"}, {"name": "bk", "display_name": "板块·领涨"}],
                 [{"pct": "<font color='red'>+2.3%</font>", "bk": "半导体　<font color='red'>北方华创</font>"},
                  {"pct": "<font color='red'>+1.8%</font>", "bk": "AI算力　浪潮信息"},
                  {"pct": "<font color='red'>+1.5%</font>", "bk": "机器人　绿的谐波"},
                  {"pct": "<font color='red'>+1.2%</font>", "bk": "低空经济　亿航智能"},
                  {"pct": "<font color='red'>+0.9%</font>", "bk": "军工　中航沈飞"}])),
             _md("💼 **持仓关联板块**"),
             _md(_mdtable(
                 [{"name": "pct", "display_name": "涨幅"}, {"name": "nb", "display_name": "持仓·所在板块"}],
                 [{"pct": "<font color='red'>+2.1%</font>", "nb": "通富微电　半导体"}])),
             # 昨日热点·今晨承接: 2列(承接·溢价|昨日热点)
             _md("🔁 **昨日热点 · 今晨承接**　强承接1 一般1 转弱1"),
             _md(_mdtable(
                 [{"name": "lv", "display_name": "承接·溢价"}, {"name": "theme", "display_name": "昨日热点"}],
                 [{"lv": "✅强承接　<font color='red'>+2.3%</font>", "theme": "⭐半导体"},
                  {"lv": "🔶一般　<font color='red'>+0.5%</font>", "theme": "光伏"},
                  {"lv": "⚠️转弱　<font color='green'>-0.3%</font>", "theme": "券商"}])),
...
register("盘面分析", "收盘复盘", "收盘后汇总当日信号命中 + 买点跟踪",
         _card_v2("📋 收盘复盘 · 06-13", "blue", [
             ...
             # 买点盈利跟踪 — 与真实 notifier._buy_tracking_tables 一致(2列 差额|名称·买点)
             _md("💹 **买点盈利跟踪**（截至 15:00）"),
             _md("**📍 今日买点 2只 · 均+1.0%（1红1绿）**"),
             _md(_mdtable(
                 [{"name": "diff", "display_name": "差额"}, {"name": "name", "display_name": "名称"}],
                 [{"diff": "+6.20%", "name": "浪潮信息 缩量突破"},
                  {"diff": "-4.20%", "name": "通富微电 回踩MA10"}])),
         ]), "交易日15:05定时推送")
...
register("盘面分析", "资金回流·板块预警", "板块涨≥1%+龙头涨停+全市场前5板块均涨≥5%",
         _card_v2("📊 资金回流·板块预警", "blue", [
             # 与真实 capital_inflow_scanner 一致: 板块头 md + 前N股表(2列 名称|涨幅) + 自选个股表
             _md("**半导体**　板块 +3.2%\n龙头 **北方华创** +9.98%(涨停)　前5股均涨 +4.5%"),
             _md("📈 板块前5股"),
             _md(_mdtable(
                 [{"name": "name", "display_name": "名称"}, {"name": "pct", "display_name": "涨幅"}],
                 [{"name": "北方华创 002371", "pct": "+9.98%"},
                  {"name": "中微公司 688012", "pct": "+5.20%"},
                  {"name": "通富微电 002156", "pct": "+2.10%"}])),
             _md("⭐ 你自选的该板块个股（1只）"),
             _md(_mdtable(
                 [{"name": "name", "display_name": "名称"}, {"name": "pct", "display_name": "涨幅"}],
                 [{"name": "通富微电 002156", "pct": "+2.10%"}])),
...
register("系统通知", "错过消息回顾", "推送开关关闭期间错过的关键信号(买/卖/减仓/急跌预警), 重新打开时汇总补发一卡",
         _card_v2("📮 错过消息回顾", "blue", [
             ...
             # 与真实 push_backfill 一致: 2列(类型前置 | 时间·标的·信号并进一格)
             _md(_mdtable(
                 [{"name": "kind", "display_name": "类型"}, {"name": "info", "display_name": "时间 · 标的 · 信号"}],
                 [{"kind": "🟢买入", "info": "06-18 10:30 国际复材 301526 缩量突破"},
                  {"kind": "🟡减仓", "info": "06-18 13:05 中际旭创 300308 接近前高"},
                  {"kind": "🔴卖出", "info": "06-18 14:12 兆易创新 603986 弱势止损"}])),
...
register("持仓研判晚报", "持仓研判晚报", "交易日前夜20:00逐股数据体检+AI次日方向性建议(持/减/清/加+目标价/止损价), 客观概率源自全市场五年同类形态前向分布",
         _card_v2("📋 持仓研判晚报", "blue", [
             ...
             # 与真实 holding_brief 一致: 2列(次日建议前置 | 个股·浮盈·目标止损·理由并进详情格)
             _md(_mdtable(
                 [{"name": "advice", "display_name": "建议"}, {"name": "info", "display_name": "个股 · 目标止损 · 理由"}],
                 [{"advice": "🟢持有", "info": "京东方A 000725 7.18·多头站均线　浮盈+12.0% 持5天　目标7.60 止损6.70　量能延续板块第二强，同类形态次日↑58%"},
                  {"advice": "🟡减仓", "info": "沪电股份 002463 151.0·高位放量滞涨　浮盈+24.0% 持11天　目标150 止损140　高位放量滞涨，同类形态次日↑47%偏弱"}])),
...
register("盘后提醒", "财报披露日历·近期", "自选/持仓里未来7天内要披露定期报告的票, 盘前提醒(防御避险)",
         _card_v2("📅 财报披露日历·近期", "blue", [
             ...
             # 与真实 disclosure_reminder 一致: 2列(股票=持仓标记+报告类型并入 | 预约披露日独占核心列)
             _md(_mdtable(
                 [{"name": "stock", "display_name": "股票"}, {"name": "day", "display_name": "预约披露日"}],
                 [{"stock": "🔴宁德时代(300750) 2025半年报", "day": "2025-08-15·3天后"},
                  {"stock": "比亚迪(002594) 2025半年报", "day": "2025-08-18·6天后"},
                  {"stock": "隆基绿能(601012) 2025半年报", "day": "2025-08-19·7天后"}])),
