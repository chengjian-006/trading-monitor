...
# ═══════════════════════════════════════════
# 3b. 尾盘破位警戒 (v1.7.582, 与卖出事件卡互补: 贴线判+连续N日标注)
# ═══════════════════════════════════════════
register("卖出信号", "尾盘破位警戒", "持仓14:40贴线判跌破MA5/MA10/MA20(同时破多档只报最深), 带连续几日尾盘破位标注, 每日尾盘都报直到收复自动消失",
         _card_v2("📉 尾盘破位警戒 · 2只持仓", "orange", [
             # 换行文本行版式(手机端不截): 每票 名称+现价 一行, 破位档标注一行(只报最深), 静音链接一行
             _md("**圣泉集团(605589)**　现价 28.30（-2.1%）\n"
                 "　破MA20·连续3日\n"       # 破5+10+20 → 只报最深MA20
                 "　[🔕 当日不提醒](#)　·　[🔕 本周不提醒](#)"),
             _md("**兆易创新(603986)**　现价 118.50（-1.4%）\n"
                 "　破MA10·今日新破\n"       # 破5+10 → 只报MA10
                 "　[🔕 当日不提醒](#)　·　[🔕 本周不提醒](#)"),
             _md("同时破多档只报最深；收复均线自动消失；破位深≥2%另有卖出信号卡。"),
         ]), "交易日尾盘14:40 · 贴线即算(现价<均线) · 同时破多档只报最深 · 连续N日从日线回算 · 收复即消失")
