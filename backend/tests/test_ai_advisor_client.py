...
async def test_narrate_returns_none_when_disabled(monkeypatch):
    monkeypatch.setattr(ac, "load_config", lambda: {"ai_advisor_enabled": False})
    def boom(*a, **k): raise RuntimeError("should not call provider when disabled")
...
async def test_narrate_passes_factsheet_as_json(monkeypatch):
    monkeypatch.setattr(ac, "load_config", lambda: {"ai_advisor_enabled": True, "ai_advisor_provider": "deepseek", "ai_model": "x"})
    captured = {}
    def fake_call(provider, system_prompt, user_content, max_tokens):
...
async def test_narrate_none_on_exception(monkeypatch):
    monkeypatch.setattr(ac, "load_config", lambda: {"ai_advisor_enabled": True, "ai_advisor_provider": "deepseek"})
